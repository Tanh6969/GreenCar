...
func RegisterVehicleRoutes(r chi.Router, vehicleSvc *service.VehicleService, log *logger.Logger) {
	...
	r.Route("/{id}", func(r chi.Router) {
		r.Get("/", handlers.GetVehicleHandler(vehicleSvc, log))		r.Get("/detail", handlers.GetVehicleDetailHandler(vehicleSvc, log))		r.Put("/", handlers.UpdateVehicleHandler(vehicleSvc, log))
